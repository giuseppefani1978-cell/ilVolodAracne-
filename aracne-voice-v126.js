(() => {
  "use strict";

  const VERSION="1.2.6";
  const bridge=window.aracneIntelligenceBridge;
  const SR=window.SpeechRecognition||window.webkitSpeechRecognition;
  if(!SR || !bridge)return;

  let recognition=null;
  let keepListening=false;
  let sessionBase="";
  let stoppedByUser=false;

  const LOCALES={it:"it-IT",fr:"fr-FR",en:"en-GB",es:"es-ES"};
  const STATUS={
    it:{ready:"Microfono spento",listening:"Ti ascolto…"},
    fr:{ready:"Microphone éteint",listening:"Je vous écoute…"},
    en:{ready:"Microphone off",listening:"Listening…"},
    es:{ready:"Micrófono apagado",listening:"Te escucho…"}
  };

  const norm=value=>String(value||"").toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g,"").replace(/[^a-z0-9\s]/g," ").replace(/\s+/g," ").trim();
  const words=value=>norm(value).split(" ").filter(Boolean);

  function language(){
    return String(bridge.getVoiceLanguage?.()||bridge.getLanguage?.()||"it").slice(0,2).toLowerCase();
  }

  function setStatus(key){
    const el=document.getElementById("assistantStatus");
    if(!el)return;
    const l=language();
    el.textContent=(STATUS[l]||STATUS.it)[key]||key;
  }

  function overlapWords(a,b){
    const wa=words(a),wb=words(b);
    const max=Math.min(wa.length,wb.length,24);
    for(let size=max;size>=2;size--){
      if(wa.slice(-size).join(" ")===wb.slice(0,size).join(" "))return size;
    }
    return 0;
  }

  function commonPrefixWords(a,b){
    const wa=words(a),wb=words(b);
    let i=0;
    while(i<wa.length && i<wb.length && wa[i]===wb[i])i++;
    return i;
  }

  function mergeSpeech(a,b){
    a=String(a||"").trim();
    b=String(b||"").trim();
    if(!a)return b;
    if(!b)return a;

    const na=norm(a),nb=norm(b);
    if(!na)return b;
    if(!nb)return a;

    // Same phrase replayed by Android/Web Speech.
    if(nb.length>=10 && (na===nb || na.endsWith(nb) || na.includes(nb)))return a;
    if(na.length>=10 && nb.includes(na))return b;

    // Session restart: Android often replays the last words from the previous
    // recognition object. Append only the non-overlapping suffix.
    const overlap=overlapWords(a,b);
    if(overlap>=2){
      const rawWords=String(b).trim().split(/\s+/);
      return (a+" "+rawWords.slice(overlap).join(" ")).replace(/\s+/g," ").trim();
    }

    // Growing hypothesis: prefer the longer alternative when both begin with
    // the same substantial prefix. This replaces the recursive v1.2.5 branch.
    const prefix=commonPrefixWords(a,b);
    const wa=words(a),wb=words(b);
    if(prefix>=3 && prefix>=Math.min(wa.length,wb.length)*0.55){
      return wb.length>=wa.length?b:a;
    }

    return (a+" "+b).replace(/\s+/g," ").trim();
  }

  function collapseSegments(segments){
    let out="";
    for(const segment of segments.map(v=>String(v||"").trim()).filter(Boolean)){
      if(!out){out=segment;continue;}
      const nOut=norm(out),nSeg=norm(segment);
      if(nSeg.length>=10 && nOut.includes(nSeg))continue;
      if(nOut.length>=10 && nSeg.includes(nOut)){out=segment;continue;}
      out=mergeSpeech(out,segment);
    }
    return out.trim();
  }

  function field(){return document.getElementById("assistantTranscript");}

  function setButtons(listening){
    const start=document.getElementById("assistantMicStart");
    const stop=document.getElementById("assistantMicStop");
    if(start)start.disabled=!!listening;
    if(stop)stop.disabled=!listening;
  }

  function stop(){
    stoppedByUser=true;
    keepListening=false;
    const active=recognition;
    recognition=null;
    if(active){
      try{active.onend=null;active.stop();}catch(error){try{active.abort();}catch(inner){}}
    }
    setButtons(false);
    setStatus("ready");
  }

  function start(){
    if(recognition)return;
    stoppedByUser=false;
    keepListening=true;
    sessionBase=String(field()?.value||"").trim();

    const r=new SR();
    recognition=r;
    r.lang=LOCALES[language()]||LOCALES.it;
    r.continuous=true;
    r.interimResults=true;
    r.maxAlternatives=1;

    r.onstart=()=>{setButtons(true);setStatus("listening");};

    r.onresult=event=>{
      const finals=[];
      const interim=[];
      // Always rebuild from the recognition object's complete result list.
      // Never append only event.resultIndex: Android may republish old finals.
      for(let i=0;i<event.results.length;i++){
        const transcript=String(event.results[i]?.[0]?.transcript||"").trim();
        if(!transcript)continue;
        (event.results[i].isFinal?finals:interim).push(transcript);
      }
      const sessionText=collapseSegments([collapseSegments(finals),collapseSegments(interim)]);
      const merged=collapseSegments([sessionBase,sessionText]);
      const target=field();
      if(target)target.value=merged;
    };

    r.onerror=event=>{
      console.warn("[Aracne Voice] recognition",event?.error||event);
      if(["not-allowed","service-not-allowed","audio-capture"].includes(event?.error))keepListening=false;
    };

    r.onend=()=>{
      if(recognition===r)recognition=null;
      setButtons(false);
      if(keepListening&&!stoppedByUser){
        sessionBase=String(field()?.value||"").trim();
        setTimeout(()=>{if(keepListening&&!recognition)start();},260);
      }else setStatus("ready");
    };

    try{r.start();}
    catch(error){
      recognition=null;
      keepListening=false;
      setButtons(false);
      setStatus("ready");
      console.warn("[Aracne Voice] start",error);
    }
  }

  function replaceButton(id,handler){
    const old=document.getElementById(id);
    if(!old)return null;
    const button=old.cloneNode(true);
    old.replaceWith(button);
    button.addEventListener("click",handler);
    return button;
  }

  replaceButton("assistantMicStart",start);
  const stopButton=replaceButton("assistantMicStop",stop);
  if(stopButton)stopButton.disabled=true;

  const originalStop=bridge.stopRecognition;
  bridge.stopRecognition=()=>{stop();try{return originalStop?.();}catch(error){return false;}};

  document.querySelector('[data-modal-close="assistantModal"]')?.addEventListener("click",stop,true);
  document.getElementById("assistantAsk")?.addEventListener("click",()=>{if(recognition)stop();},true);

  window.AracneVoice126={version:VERSION,start,stop,mergeSpeech,collapseSegments,status:()=>({version:VERSION,listening:!!recognition,keepListening})};
  console.info(`[Aracne Voice] v${VERSION} active — cross-platform speech deduplication`);
})();
